...
# TODO: scrap this? use the same architecture for wm?
...
class TransitionModel(nn.Module):
    def __init__(
        ...
        unimix_ratio=0.01,
        initial="learned",
        num_actions=None,  # TODO: HORRIBLE, action space
        embed=None,  # TODO: HORRIBLE, state space i think
        device=None,  # TODO: HORRIBLE
    ):
        super().__init__()
        self._stoch = stoch
        self._deter = deter
    ...
    def initial(self, batch_size):
        """
        Initiates initial state, based on batch size
        """
    ...
    def observe(self, embed, action, is_first, prev_state=None):
        ...
        Args:
            - embed: Encoded inputs from encoder
            - action:
            - is_first:
            - prev_state: If any, previous recurrent state of model
        """
        # swap batch & time dim (batch, time, ch) -> (time, batch, ch)
        swap = lambda x: x.permute([1, 0] + list(range(2, len(x.shape))))

        embed, action, is_first = swap(embed), swap(action), swap(is_first)
        
        # NOTE: prev_state[0] means selecting posterior of return(posterior, prior) from obs_step
    ...
    def imagine_with_action(self, action, state):
        ...
        action = swap(action)
        prior = utils.static_scan(self.imagine_step, [action], state)
        prior = prior[0]
        prior = {k: swap(v) for k, v in prior.items()}
        return prior
    ...
    def get_dist(self, state, dtype=None):
        if self._discrete:
            ...
            dist = torchd.independent.Independent(
                utils.OneHotDist(logit, unimix_ratio=self._unimix_ratio), 1
            )
        else:
    ...
    def imagine_step(self, prev_state, prev_action, sample=True):
        """
        Predicts the next state based on the previous, via the equation
        h_t = f_phi(h_t-1, z_t-1, a_t-1).

        Args:
            - prev_state: Dictionary containing 'stoch' and 'deter' keys.
                stoch is z_t-1, deter is h_t-1
            - prev_action: torch.Tensor of previous action.
        """
    ...
    def kl_loss(self, post, prior, free, dyn_scale, rep_scale):
        raise NotImplementedError('kl loss moving out of this module')
