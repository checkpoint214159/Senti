...
class ActionDecoder(torch.nn.Module):
    ...
    def forward(self, hidden, lookup):
        ...
        actions = []
        # assert action_targets["inventory_give_item"].sum() <= hidden.shape[0]
        # assert action_targets["inventory_give_player"].sum() <= hidden.shape[0]
        # assert action_targets["gold_target"].sum() <= hidden.shape[0]
        # assert action_targets["gold_quantity"].sum() <= hidden.shape[0]
