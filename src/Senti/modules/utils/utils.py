"""
Utilities expressed under 'tools.py' at https://github.com/NM512/dreamerv3-torch.git
All credit goes to them
TODO update this if we throw in other utility functions here too, also organise if needbe
"""
...
def static_scan(fn, inputs, initializer):
    """
    Scan function similar to that of tf.scan. PyTorch, why dont you have one bro
    This iterates over the first dimension of each input and feeds it to fn.
    E.g, 
    inputs = {
        P: (A, B, C, ...),
        Q: (A, B, C, ...),
    ...
    Args:
        - fn: Function to run. Returns must be of the same type as initializer.
        - inputs: Dict or List of torch.Tensors.
        - initializer: Initial input to fn, has to be first positional arg.
    Return:
        - outputs: Dictionary of {index: fn return}.
            e.g {0: [A, B, C, ...], 1: [1, B, C, ...], ...}
    """
    last = initializer  # dict or list[dict] or list[list]
    indices = inputs[0].shape[0]  # first dim
    if isinstance(last, list) or isinstance(last, dict):
        outputs = {}
    else:
        raise AssertionError('Assertion failed. Initializer must be a list or dict.')
    
    for i, index in enumerate(range(indices)):
        inp = lambda x: (_input[x] for _input in inputs)  # retrieve tuple at timestep  # noqa: E731
        last = fn(last, *inp(index))  # obs_step(self, prev_state, prev_action, embed, is_first, sample=True)
        if i == 0:
